...
from server.defines import settings
from server.defines.User import User
from server.defines.Config import Config
...
class Server:
    ...
    def __init__(self):
        ...
        if not os.path.exists("./config.yml"):
            open("./config.yml", "w").write(settings.default_config)
        with open("./config.yml", "r") as f:
            yaml_text = f.read()
            config_data = yaml.load(yaml_text, yaml.Loader)
        self.config = Config(config_data["ip_address"], config_data["network_port"], config_data["default_room"],
                             config_data["log"], config_data["record"], config_data["force_account"],
                             config_data["allow_register"], config_data["lock_server"])
        print(f'Lhat Chatting Server Version {self.VERSION} using AGPL v3.0 License')
        logger.remove()
        format = "<g>{time:YYYY-MM-DD HH:mm:ss}</g> [<lvl>{level}</lvl>] <c>{name}</c> | {message}"
        if self.config.log:
            logger.add(open(f'logs/lhat_server{time.strftime("%Y-%m-%d", time.localtime())}.log', 'a'),
                       format=format)
        logger.add(stderr, format=format)
        self.loggable = self.config.log
        self.recordable = self.config.record
        self.force_account = self.config.force_account
        self.allow_register = self.config.allow_register
        self.lock_server = self.config.lock_server
        self.ip = self.config.ip_address
        self.port = self.config.network_port
        self.default_room = self.config.default_room

        logger.info('Server arguments set.')
        logger.info('=====NEW SERVER INITIALIZING BELOW=====', show_time=False)
        ...
        if 'root' not in self.sql_exist_user:  # 如果数据库中没有root用户，则创建
            ...
            logger.info('Root account not found, created.')
    ...
    def run(self):
        ...
        logger.info('================================', show_time=False)
        logger.info(f'Running server on {self.ip}:{self.port}')
        logger.info('  To change the settings, \n  please visit settings.py')
        if self.force_account:
            logger.warning('Warning, force account is enabled!!! \n'
        ...
        if mask & selectors.EVENT_READ:  # 如果可读，则开始从客户端读取消息
            ...
            if data.inbytes:  # 如果消息列表不为空
                try:
                    ...
                    logger.info('A message is not in utf-8 encoding.')
                else:
    ...
    def processMessage(self, message: str, sock: socket.socket, address=None):
        ...
        if recv_data[0] == 'TEXT_MESSAGE' or recv_data[0] == 'COLOR_MESSAGE':  # 如果能正常解析，则进行处理
            ...
            try:
                if command[0] == 'room':
                    ...
                    if command[1] == 'create':  # 创建聊天室，需要Manager以上权限
                        logger.info(f'{recv_data[1]} wants to create room {room_name}')
                        if self.user_connections[recv_data[1]].getPermission() != 'User':  # 如果不是普通用户
                            if room_name in self.chatting_rooms or room_name in self.user_connections:
                                logger.info(f'Room {room_name} already exists, abort creating.')
                            ...
                            logger.info(f'User {recv_data[1]} is not allowed to create room.')
                            sock.send(pack(f'你没有创建聊天室的权限。', 'Server', '', 'TEXT_MESSAGE'))
                    elif command[1] == 'join':  # 加入聊天室
                        logger.info(f'{recv_data[1]} join room {room_name}')
                        if room_name in self.chatting_rooms:
                            ...
                            logger.info(f'Room {room_name} does not exist, abort joining.')
                            sock.send(pack(f'{room_name} 不存在，无法加入。', 'Server', '', 'TEXT_MESSAGE'))
                    elif command[1] == 'list':  # 列出所有聊天室
                        logger.info(f'{recv_data[1]} wants to check online rooms.')
                        ...
                        logger.info(f'{recv_data[1]} wants to leave room {room_name}')
                        if room_name in self.chatting_rooms:
                            for name, user in self.user_connections.items():
                                if name == recv_data[1]:
                            ...
                            logger.info(f'Room {room_name} does not exist, abort leaving.')
                            sock.send(pack(f'{room_name} 不存在，无法退出。', 'Server', '', 'TEXT_MESSAGE'))
                    elif command[1] == 'delete':  # 删除聊天室，需要Manager以上权限
                        logger.info(f'{recv_data[1]} wants to delete room {room_name}')
                            ...
                            if room_name in self.chatting_rooms:
                                ...
                                for user in self.user_connections.values():
                                    if room_name in user.getRooms():
                                        user.removeRoom(room_name)
                                        user.getSocket().send(
                                            pack(f'{room_name} 聊天室已被管理员删除，已自动退出本聊天室。',
                                                 'Server', '', 'TEXT_MESSAGE'))
                                    sock.send(pack(f'Room {room_name} deleted.', 'Server', '', 'TEXT_MESSAGE'))
                            else:
                                logger.info(f'Room {room_name} does not exist, abort deleting.')
                                sock.send(pack(f'{room_name} 不存在，无法删除。', 'Server', '', 'TEXT_MESSAGE'))
                        else:
                            logger.info(f'{recv_data[1]} do not have the permission to delete {room_name}.')
                    ...
                    if self.user_connections[recv_data[1]].getPermission() == 'Admin':
                        if command[1] == 'add':
                            logger.info(f'{recv_data[1]} wants to add {operate_user} to the Manager group.')
                            if operate_user in self.user_connections and \
                                ...
                                logger.info(f'{operate_user} permission changed to Manager.')
                            ...
                            logger.info(f'{recv_data[1]} wants to delete {operate_user} from the Manager group.')
                            if operate_user in self.user_connections and \
                                    self.user_connections[operate_user].getPermission() == 'Manager':
                                self.user_connections[operate_user].setPermission('User')
                                logger.info(f'{operate_user} permission changed to User.')
                                self.user_connections[operate_user].getSocket().send(
                            ...
                            logger.info(f'{recv_data[1]} wants to list all managers.')
                        ...
                        if command[1] == 'list':
                            ...
                            logger.info(f'{recv_data[1]} do not have the permission to manage users.')
                            sock.send(pack(f'你没有最高权限以用于管理用户。', 'Server', '', 'TEXT_MESSAGE'))

                elif command[0] == 'kick':
                    logger.info(f'{recv_data[1]} wants to kick {command[1]}.')
                    if self.user_connections[recv_data[1]].getPermission() != 'User':
                        if command[1] in self.user_connections and \
                            ...
                            logger.info(f'{command[1]} kicked.')
                            sock.send(pack(f'{command[1]} kicked.', 'Server', '', 'TEXT_MESSAGE'))
                        else:
                            if recv_data[1] == command[1]:
                                logger.info(f'{recv_data[1]} tried to kick himself.')
                                ...
                                logger.info(f'{command[1]} does not exist, abort kicking.')
                                sock.send(pack(f'{command[1]} does not exist or is a Manager, abort kicking.',
                                               'Server', '', 'TEXT_MESSAGE'))
                    else:
                        logger.info(f'{recv_data[1]} do not have the permission to kick {command[1]}.')
                        sock.send(pack(f'You do not have the permission to kick {command[1]}.',
                                       'Server', '', 'TEXT_MESSAGE'))

                elif command[0] == 'update':  # 有的用户可能无法及时更新用户列表，所以可以手动更新
                    logger.info(f'{recv_data[1]} wants to update his user manifest manually.')
                    ...
                    logger.info(f'{recv_data[1]} wants to operate the SQL database.')
                    if self.user_connections[recv_data[1]].getPermission() == 'Admin':
                        if command[1] == 'create':  # 创建用户
                            logger.info(f'{recv_data[1]} wants to create {command[2]}.')
                            if command[2] not in self.sql_exist_user and \
                                ...
                                logger.info(f'{command[2]} created, permission: {command[3]}.')
                                sock.send(pack(f'{command[2]} created, permission: {command[3]}.',
                                               'Server', '', 'TEXT_MESSAGE'))
                            else:
                                logger.info(f'{command[2]} already exists.')
                                sock.send(pack(f'{command[2]} already exists.', 'Server', '', 'TEXT_MESSAGE'))

                        elif command[1] == 'setpwd':  # 设置密码
                            logger.info(f'{recv_data[1]} wants to set password of {command[2]}.')
                            if command[2] in self.sql_exist_user:
                                ...
                                logger.info(f'{command[2]} does not exist.')
                                sock.send(pack(f'{command[2]} does not exist.', 'Server', '', 'TEXT_MESSAGE'))

                        elif command[1] == 'setper':  # 设置权限
                            logger.info(f'{recv_data[1]} wants to set permission of {command[2]}.')
                            if command[2] == 'root':
                                logger.info(f'{recv_data[1]} tried to set permission of root.')
                            ...
                            logger.info(f'{recv_data[1]} wants to delete {command[2]}.')
                            # 查看数据库中要删除的用户的权限
                            ...
                            logger.info(f'{recv_data[1]} wants to ban {command[2]}')
                            self.sql_cursor.execute('SELECT PERMISSION FROM USERS WHERE USER_NAME = ?', (command[2],))
                            ...
                            logger.info(f'{recv_data[1]} wants to restore {command[2]}')
                            if command[2] in self.sql_exist_user:
                            ...
                            logger.info(f'{command[1]} is not a valid operation.')
                            sock.send(pack(f'{command[1]} is not a valid operation.', 'Server', '', 'TEXT_MESSAGE'))

                elif command[0] == 'option':  # 服务器管理设置
                    if command[1] == 'show':
                        logger.info(f'{recv_data[1]} checked the server options.')
                        ...
                        logger.info(f'{recv_data[1]} tried to set {command[2]} to {command[3]}')
                        vaild_option = True
                    ...
                    logger.info(f'{recv_data[1]} wants to reset password.')
                    if not command[1]:
                        logger.info(f'{recv_data[1]} tried to reset password without a password.')
                        ...
                        logger.info(f'{recv_data[1]} does not exist.')
                ...
                logger.info(f'There is something wrong with {recv_data[1]} command.')
            ...
            logger.info(f'New register information received.')
            ...
            try:
                ...
                logger.info('This is not a valid register information.')
                sock.send(bytes('failed\0', 'utf-8'))
                self.closeConnection(sock, address)
                return
            if passwd:
                if user in self.user_connections:
                    logger.info(f'{user} tried to register again.')
                    sock.send(bytes('failed\0', 'utf-8'))  # 注册信息无法重复
                    self.closeConnection(sock, address)
                    return
            else:
                logger.info(f'{user} tried to register without password.')
                sock.send(bytes('failed\0', 'utf-8'))  # 如果没有密码，则返回失败
                self.closeConnection(sock, address)
                return
            if user in self.sql_exist_user:
                logger.info(f'{user} is already in the database.')
                sock.send(bytes('failed\0', 'utf-8'))  # 如果用户已存在，则返回失败
                self.closeConnection(sock, address)
                return
            elif user == 'Server':
                logger.info(f'{user} tried to register as Server.')
                sock.send(bytes('failed\0', 'utf-8'))  # 不允许注册为Server
                self.closeConnection(sock, address)
                return
            elif len(user) > 20:
                logger.info(f'{user} tried to register with a over-length name.')
                sock.send(bytes('failed\0', 'utf-8'))  # 不允许注册为Server
                self.closeConnection(sock, address)
                return
            else:
                ...
                logger.info(f'{user} has been registered.')
    ...
    def processNewLogin(self, sock, address, user_info):
        ...
        if passwd:
            if user in self.user_connections:
                logger.info(f'{user} tried to login again.')
            ...
            try:
                ...
                logger.info(f'{user} tried to login with a wrong password.')
            ...
            logger.info(f'{user} tried to login without password.')
            ...
            logger.info(f'{user} is already in the online list.')
            sock.send(pack(f'该用户名已存在于在线列表。', 'Server', '', 'KICK_NOTICE'))
            self.closeConnection(sock, address)
            return
        elif user == 'Server':
            logger.info(f'{user} tried to login as Server.')
        ...
        if query_result[3]:
            logger.info(f'{user} is banned.')
            sock.send(pack(f'你已被管理员封禁。', 'Server', '', 'KICK_NOTICE'))
            self.closeConnection(sock, address)
            return
        elif query_result[2] != 'Admin' and self.lock_server:
            logger.info(f'{user} tried to login the locked server without an Admin permission.')
        ...
        if logged_user and query_result:
            ...
            self.user_connections[user] = \
                User(sock, address, query_result[2], self.client_id, user, "a")
        else:
            self.user_connections[user] = \
                User(sock, address, 'User', self.client_id, user, "a")  # 将用户名和连接加入连接列表
    ...
    def closeConnection(self, sock: socket.socket, address: tuple):
        ...
        :return: 无返回值
        """
        logger.info(f'Connection closed: {address[0]}:{address[1]}')  # 日志
